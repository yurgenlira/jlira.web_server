...
## Testing (Mandatory)
- **Create Tests**: Every role must have Molecule tests.
- **Scenario Naming**: Create a scenario for each role (molecule scenario should match the role name).
  - Example: For role `certificates`, create scenario `extensions/molecule/certificates/`
- **Test File Organization**: The test file name in `extensions/molecule/<role>/tests/` must match the task file name in `roles/<role>/tasks/`.
  - Task file: `roles/apache/tasks/configure.yml` → Test file: `extensions/molecule/apache/tests/configure.yml`
- **Test Structure**: Define scenarios, platforms, verifiers, etc. in `molecule.yml`
- **Coverage**: Test all major use cases and edge cases defined in requirements
